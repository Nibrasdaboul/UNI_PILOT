...
export default function Courses() {
  ...
  const enrolledCatalogIds = new Set(
    (Array.isArray(courses) ? courses : []).map((c) => Number(c.catalog_course_id)).filter((id) => !Number.isNaN(id) && id > 0)
  );
  const canEnroll = (catalogCourse) => {
    ...
    return enrolledCatalogIds.has(prereqId);
  };
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="relative w-64">
            ...
            <Input 
              ...
              className="pl-10 rtl:pl-3 rtl:pr-10 rounded-xl bg-background border-primary/10 h-11"
              value={search}
      ...
      {catalogSorted.length > 0 && (
        <Card className="rounded-2xl sm:rounded-[2rem] border shadow-sm overflow-hidden bg-card/50 backdrop-blur-sm">
          <CardHeader className="border-b bg-muted/20 px-6 py-6">
            ...
            <CardDescription>
              {language === 'ar' ? 'اختر المواد من مقررات الجامعة حسب ترتيب الشجرة (المتطلب السابق يجب أن يكون مسجلاً)' : 'Choose from university catalog; enroll in order (previous course must be enrolled first)'}
            </CardDescription>
          </CardHeader>
                    ...
                    <CardContent className="p-4 flex items-start justify-between gap-2">
                      ...
                      <div className="shrink-0">
                        {alreadyEnrolled ? (
                          ...
                          <span className="flex items-center gap-1 text-xs text-muted-foreground" title={t('courses.prerequisiteRequired')}>
