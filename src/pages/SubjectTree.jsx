...
export default function SubjectTree() {
  ...
  const catalogSorted = topologicalOrder(catalogList, buildCatalogById(catalogList));
    ...
    <div className="space-y-6 pb-12" data-testid="subject-tree-page" ref={containerRef}>
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {language === 'ar'
              ? 'كل مواد الكتالوج — المسجلة مفتوحة، غير المكتمل متطلباتها مقفولة'
              : 'All catalog courses — enrolled are open; locked until prerequisites are met'}
          </p>
        </div>
