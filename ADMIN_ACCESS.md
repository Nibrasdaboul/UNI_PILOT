...
- **Locally:** run `npm run seed` once, then use the table above.
- **On Render:** these users are created automatically on first deploy (when the database is empty). Use the same credentials to log in as admin.

---

## Option 1: With backend (Node.js + SQLite) — local
